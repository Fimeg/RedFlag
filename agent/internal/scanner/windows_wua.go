...
func (s *WindowsUpdateScannerWUA) Name() string { return "Windows Update Scanner" }
...
func (s *WindowsUpdateScannerWUA) Scan() ([]client.UpdateReportItem, error) {
	...
	// TODO(DEV-031): Ghost update prevention — filter scan results
	// against recently-installed update IDs where RebootRequired=true.
	// Currently the RebootRequired flag is reported to the server
	// but the scanner still returns the update as available until
	// Windows commits the install state.
	// See docs/C1_Verification_Report.md Part 4 for full context.
