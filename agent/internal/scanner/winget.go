...
func (s *WingetScanner) Name() string { return "Winget Package Update Scanner" }
...
func (s *WingetScanner) IsAvailable() bool {
	...
	_, err := s.findWingetPath()
	return err == nil
}

// findWingetPath searches PATH and known system-wide install locations (F-C1-1 fix).
// This ensures winget is found when running as SYSTEM service account.
func (s *WingetScanner) findWingetPath() (string, error) {
	// 1. Check PATH first (fastest, works for user-space installs)
	if path, err := exec.LookPath("winget"); err == nil {
		return path, nil
	}

	// 2. Check known system-wide install locations (accessible to SYSTEM account)
	// TODO: winget v1.6+ required for --output json flag
	knownPaths := []string{
		`C:\Windows\System32\winget.exe`,
	}

	// 3. Glob for versioned WindowsApps directory
	globPattern := `C:\Program Files\WindowsApps\Microsoft.DesktopAppInstaller_*\winget.exe`
	if matches, err := filepath.Glob(globPattern); err == nil {
		knownPaths = append(knownPaths, matches...)
	}

	for _, p := range knownPaths {
		if _, err := os.Stat(p); err == nil {
			log.Printf("[INFO] [agent] [scanner] winget_found_at path=%q", p)
			return p, nil
		}
	}

	log.Printf("[INFO] [agent] [scanner] winget_not_found checked_path_and_known_locations")
	return "", fmt.Errorf("winget not found in PATH or known locations")
}
...
func (s *WingetScanner) Scan() ([]client.UpdateReportItem, error) {
	...
	if updates, err := s.scanWithJSON(); err == nil {
		...
		lastErr = err
		log.Printf("Winget JSON scan failed: %v\n", err)
	}
	...
	if updates, err := s.scanWithBasicOutput(); err == nil {
		...
		log.Printf("Winget basic scan failed: %v\n", err)
	}

	// Method 3: Attempt automatic recovery for known issues
	if isKnownWingetError(lastErr) {
		log.Printf("Attempting automatic winget recovery...\n")
		if updates, err := s.attemptWingetRecovery(); err == nil {
			log.Printf("Winget recovery successful, found %d updates\n", len(updates))
			return updates, nil
		} else {
			log.Printf("Winget recovery failed: %v\n", err)
		}
...
// parseWingetTextOutput parses winget text output using column-position parsing (F-C1-2 fix).
// Handles package names with spaces by using the header separator line to determine column widths.
func (s *WingetScanner) parseWingetTextOutput(output string) ([]client.UpdateReportItem, error) {
	...
	if separatorIdx < 1 || separatorIdx >= len(lines)-1 {
		// Fall back to simple parsing if no separator found
		return s.parseWingetTextSimple(output)
	}
	...
	// Find column start positions by locating header keywords
	colNames := []string{"Name", "Id", "Version", "Available"}
	colStarts := make([]int, 0, len(colNames))
	...
	if len(colStarts) < 3 {
		// Not enough columns found, fall back
		return s.parseWingetTextSimple(output)
	}
	...
	for i := separatorIdx + 1; i < len(lines); i++ {
		...
		var cols []string
		for j, start := range colStarts {
			if start >= len(line) {
				cols = append(cols, "")
				continue
			}
			end := len(line)
			if j+1 < len(colStarts) {
				end = colStarts[j+1]
				if end > len(line) {
					end = len(line)
				}
		...
		// Column order: Name, [Id], Version, Available
		pkgName := cols[0]
		currentVersion := cols[len(cols)-2]
		availableVersion := cols[len(cols)-1]

		if availableVersion == currentVersion || availableVersion == "Unknown" || availableVersion == "" {
			continue
		}
...
// parseWingetTextSimple is the legacy simple parser for outputs without clear column headers
func (s *WingetScanner) parseWingetTextSimple(output string) ([]client.UpdateReportItem, error) {
	var updates []client.UpdateReportItem
	lines := strings.Split(output, "\n")
	...
	for _, line := range lines {
		...
		if len(fields) >= 3 {
			pkgName := fields[0]
			currentVersion := fields[len(fields)-2]
...
func (s *WingetScanner) resetWingetSources() error {
	...
	if err != nil {
		log.Printf("Failed to reset winget sources: %v\n", err)
		return err
	}
	...
	_, err = cmd.CombinedOutput()
	if err != nil {
		log.Printf("Failed to add winget source: %v\n", err)
		return err
	}
...
func (s *WingetScanner) updateWingetSilent() error {
	...
	_, err := cmd.CombinedOutput()
	if err != nil {
		log.Printf("Failed to update winget: %v\n", err)
		return err
	}
	return nil
}
...
func (s *WingetScanner) repairWindowsAppInstaller() error {
	...
	_, err := cmd.CombinedOutput()
	if err != nil {
		log.Printf("Failed to repair Windows App Installer: %v\n", err)
		return err
	}
	return nil
}
