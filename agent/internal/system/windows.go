...
func getWindowsCPUInfo() (*CPUInfo, error) {
	...
	if cmd, err := exec.LookPath("wmic"); err == nil {
		...
		if data, err := exec.Command(cmd, "cpu", "get", "Name").Output(); err == nil {
			...
			log.Printf("[ERROR] [agent] [windows_cpu_info] wmic_name_failed error=%v", err)
		}
		...
		if data, err := exec.Command(cmd, "cpu", "get", "NumberOfCores").Output(); err == nil {
			...
			log.Printf("[ERROR] [agent] [windows_cpu_info] wmic_cores_failed error=%v", err)
		}
		...
		if data, err := exec.Command(cmd, "cpu", "get", "NumberOfLogicalProcessors").Output(); err == nil {
			...
			log.Printf("[ERROR] [agent] [windows_cpu_info] wmic_threads_failed error=%v", err)
		}
		...
		log.Printf("[ERROR] [agent] [windows_cpu_info] wmic_not_found error=%v", err)
	}

	// Fallback to PowerShell if wmic failed
	if cpu.ModelName == "" {
		log.Printf("[INFO] [agent] [windows_cpu_info] falling_back_to_powershell")
		if psCmd, err := exec.LookPath("powershell"); err == nil {
			if data, err := exec.Command(psCmd, "-NoProfile", "-NonInteractive", "-Command", "Get-CimInstance -ClassName Win32_Processor | Select-Object -First 1 Name,NumberOfCores,NumberOfLogicalProcessors | ConvertTo-Json").Output(); err == nil {
				parsed, parseErr := parseWindowsCPUInfoJSON(data)
				if parseErr != nil {
					log.Printf("[ERROR] [agent] [windows_cpu_info] powershell_parse_failed error=%v", parseErr)
				} else {
					cpu = parsed
				}
			} else {
				log.Printf("[ERROR] [agent] [windows_cpu_info] powershell_fallback_failed error=%v", err)
			}
		}
	}

	log.Printf("[INFO] [agent] [windows_cpu_info] model=%q cores=%d threads=%d", cpu.ModelName, cpu.Cores, cpu.Threads)
